...
public class JdbcVoteAnswerRepository implements VoteAnswerRepository{
    ...
    public int voteCount(Answer answer) {
        int up = template.query("SELECT * FROM VoteAnswer WHERE Answer_idAnswer = ? AND VoteType = true",
                new VoteAnswerMapper(),answer.getId()).size();
        int down = template.query("SELECT * FROM VoteAnswer WHERE Answer_idAnswer = ? AND VoteType = false",
                new VoteAnswerMapper(),answer.getId()).size();
        return up - down;
    }
    ...
    private Integer insert(VoteAnswer voteAnswer){
        ...
        data.put("Answer_idAnswer",voteAnswer.getIdAnswer());
        data.put("User_Username",voteAnswer.getAuthor());
